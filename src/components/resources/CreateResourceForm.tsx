...
const CreateResourceForm: React.FC<Props> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !content) {
      alert(t("createResource.validationError"));
      return;
    }
    ...
    try {
      ...
      alert(t("createResource.successMessage"));
      setTitle("");
      setContent("");
      onClose();
      onSuccess();
      ...
      console.error(err);
      alert(t("createResource.errorMessage"));
    }
  };
  ...
  return (
    ...
    >
      <h3 className="text-lg font-semibold">{t("createResource.title")}</h3>

      <div className="bg-blue-50 border border-blue-200 rounded p-2 text-sm">
        <span className="font-medium">
          {t("createResource.resourceLanguage")}:
        </span>{" "}
        {i18n.language === "pl"
          ? t("createResource.polish")
          : t("createResource.english")}
      </div>

      <input
        type="text"
        placeholder={t("createResource.titlePlaceholder")}
        value={title}
        onChange={(e) => setTitle(e.target.value)}
        className="border p-2 rounded w-full"
      />
      <textarea
        placeholder={t("createResource.contentPlaceholder")}
        value={content}
