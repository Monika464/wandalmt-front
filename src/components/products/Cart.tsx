...
//   const handleCheckout = async () => {
//     if (!user) {
//       // Brak zalogowanego → przekierowanie
//       navigate("/login");
//       return;
//     }
//     try {
...
//         },
//         body: JSON.stringify({ items }),
//         credentials: "include", // jeśli używasz cookie z auth
//       });
//       const data = await res.json();
...
//         // Przekierowanie do strony powrotnej po zakończeniu płatności
//         window.location.href = `http://localhost:5173/cart-return?session_id=${data.client_secret}`;
//         //window.location.href = `https://checkout.stripe.com/pay/${data.client_secret}`;
//       } else {
//         console.error("Błąd utworzenia sesji:", data.error);
//       }
//     } catch (err) {
//       console.error("Checkout error:", err);
//     } finally {
...
//           className="bg-green-500 px-4 py-2 rounded text-white hover:bg-green-600"
//         >
//           {loading ? "Przekierowanie do platnosci..." : "Zapłać za koszyk"}
//         </button>
...
// //       </button>
// //       <button
// //         onClick={() => navigate("/cart-checkout", { state: { cart: items } })}
