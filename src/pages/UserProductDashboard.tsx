...
//   // ✅ UŻYJ TYLKO JEDNEJ DEFINICJI - tej z useMemo
//   const purchasedProducts = React.useMemo(() => {
//     if (!userOrders || userOrders.length === 0) return [];

//     // Filtruj aktywne zamówienia (paid lub partially_refunded)
//     const activeOrders = userOrders.filter(
//       (order) =>
//         order.status === "paid" || order.status === "partially_refunded",
//     );
...
//           // Pomiń produkty, które są w pełni zwrócone
//           if (isFullyRefunded) return;

//           const product = item.product || item;
//           // Użyj productId lub _id do unikalności
//           const productId = product.productId || product._id;
...
//               purchasedDate: order.createdAt,
//               chapters: product.chapters || [],
//               // Info o częściowym zwrocie jeśli potrzebne
//               isPartiallyRefunded:
//                 refundQuantity > 0 && refundQuantity < item.quantity,
//               refundedQuantity: refundQuantity,
//               originalQuantity: item.quantity,
...
//     console.log("📦 Purchased products after refund:", allProducts);
//     return allProducts;
//   }, [userOrders]); // Zależność od userOrders
...
//                   ⚠️ Częściowo zwrócone ({product.refundedQuantity}/
//                   {product.originalQuantity})
//                 </div>
//               )}
//             </div>
...
// import { useDispatch, useSelector } from "react-redux";
// import { fetchUserOrders } from "../store/slices/orderSlice";
// import { selectUserProducts } from "../store/slices/orderSlice";
...
//       if (order.products && Array.isArray(order.products)) {
//         order.products.forEach((item) => {
//           // 🔥 WAŻNE: Sprawdź czy produkt NIE JEST zwrócony!
//           const refundQuantity = item.refundQuantity || 0;
//           const isFullyRefunded = refundQuantity >= item.quantity;
...
//               purchasedDate: order.createdAt,
//               chapters: product.chapters || [],
//               // Dodaj info o częściowym zwrocie jeśli potrzebne
//               isPartiallyRefunded:
//                 refundQuantity > 0 && refundQuantity < item.quantity,
//               refundedQuantity: refundQuantity,
//               originalQuantity: item.quantity,
